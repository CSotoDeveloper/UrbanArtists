    ...
    <div id='art' className='flex flex-col items-center gap-4 my-16 md:mx-10 text-white'>
      ...
      <div className='w-full grid grid-cols-2 sm:grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {artists.slice(0, 10).map(artist => (
          <div onClick={()=> {navigate(`/appointment/${artist._id}`);scrollTo(0,0)}} 
          className=' border border-primary bg-stone-950 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' 
          key={artist._id}>
      ...
      className='bg-primary font-semibold text-white px-12 py-3 rounded-full mt-10' type='button'>Ver mas</button>
    </div>
  )
}
